...
export const preset: Types.OutputPreset<NearOperationFileConfig> = {
  buildGeneratesSection: (options) => {
    ...
    const rootTypeMap = getRootTypeMap(schemaObject);
      ...
      for (const field in rootType.getFields()) {
        const operationDefinitionNode: OperationDefinitionNode =
          buildOperationNodeForField({
            ...
            kind,
            field,
            depthLimit: 3, // config.depthLimit,
            circularReferenceDepth: 1, // config.circularReferenceDepth,
          });
    ...
    for (const [filename, record] of filePathsMap.entries()) {
      ...
      if (importAllFragmentsFrom) {
        fragmentImportsArr = record.fragmentImports.map<
      ...
      for (const source of record.documents) {
        ...
        (combinedSource.document.definitions as any).push(
